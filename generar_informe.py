...
def cabecera_tabla(pdf_obj, cols):
    ...
    for ancho, texto in cols:
        pdf_obj.cell(w=ancho, h=10, text=texto, border=1, new_x="RIGHT", new_y="TOP", align="C", fill=True)
    pdf_obj.ln()
...
if not misiones:
    ...
    pdf.cell(w=0, h=10, text="ERROR: No hay misiones en la ontología", border=0, new_x="LMARGIN", new_y="NEXT", align="L")
...
for mision in misiones:
    ...
    pdf.set_font("Helvetica", "B", 16)
    pdf.cell(w=0, h=15, text=f"  INFORME DE RIESGOS - MISION: {nombre_m}",
    ...
    pdf.cell(w=0, h=10, text="1. EVALUACION GLOBAL DE LA MISION", border=0, new_x="LMARGIN", new_y="NEXT", align="L")

    pdf.set_font("Helvetica", "B", 11)
    pdf.cell(w=70, h=10, text="Riesgo Inherente:", border=0, new_x="RIGHT", new_y="TOP", align="L")
    aplicar_color_fondo(pdf, ei_m)
    pdf.cell(w=45, h=10, text=f"{ei_m} ({si_m:.2f})", border=1, new_x="LMARGIN", new_y="NEXT", align="C", fill=True)

    pdf.set_text_color(0, 0, 0)
    pdf.cell(w=70, h=10, text="Riesgo Residual:", border=0, new_x="RIGHT", new_y="TOP", align="L")
    aplicar_color_fondo(pdf, er_m)
    pdf.cell(w=45, h=10, text=f"{er_m} ({sr_m:.2f})", border=1, new_x="LMARGIN", new_y="NEXT", align="C", fill=True)
    pdf.ln(10)
    ...
    pdf.set_text_color(0, 0, 0)
    pdf.set_font("Helvetica", "B", 12)
    pdf.cell(w=0, h=10, text="2. NODOS DE ATENCION PRIORITARIA", border=0, new_x="LMARGIN", new_y="NEXT", align="L")
    ...
    if not criticos:
        ...
        pdf.cell(w=190, h=10, text="No hay nodos en riesgo alto/critico en esta mision.",
                 border=1, new_x="LMARGIN", new_y="NEXT", align="C")
    else:
        for n in criticos:
            pdf.set_text_color(0, 0, 0)
            pdf.set_font("Helvetica", "", 8)
            pdf.cell(w=20, h=10, text=n["tipo"],           border=1, new_x="RIGHT", new_y="TOP", align="C")
            pdf.cell(w=50, h=10, text=truncar(n["nombre"]), border=1, new_x="RIGHT", new_y="TOP", align="C")

            pdf.set_font("Helvetica", "B", 8)
            aplicar_color_texto(pdf, n["ei"])
            pdf.cell(w=35, h=10, text=f"{n['ei']} ({n['si']:.2f})", border=1, new_x="RIGHT", new_y="TOP", align="C")

            pdf.set_text_color(80, 80, 80)
            pdf.set_font("Helvetica", "", 8)
            pdf.cell(w=50, h=10, text=n["mitig"], border=1, new_x="RIGHT", new_y="TOP", align="C")

            pdf.set_font("Helvetica", "B", 8)
            aplicar_color_texto(pdf, n["er"])
            pdf.cell(w=35, h=10, text=f"{n['er']} ({n['sr']:.2f})", border=1, new_x="LMARGIN", new_y="NEXT", align="C")
    ...
    pdf.set_text_color(0, 0, 0)
    pdf.set_font("Helvetica", "B", 12)
    pdf.cell(w=0, h=10, text="3. AMENAZAS DETECTADAS EN ACTIVOS", border=0, new_x="LMARGIN", new_y="NEXT", align="L")
    ...
    if not activos_con_amenazas:
        pdf.set_font("Helvetica", "", 8)
        pdf.cell(w=190, h=10,
                 text="No hay incidentes/amenazas registrados en activos de esta mision.",
                 border=1, new_x="LMARGIN", new_y="NEXT", align="C")
    else:
        pdf.set_font("Helvetica", "", 8)
        for act in activos_con_amenazas:
            pdf.cell(w=60,  h=10, text=truncar(act["nombre"],  35), border=1, new_x="RIGHT", new_y="TOP", align="C")
            pdf.cell(w=130, h=10, text=truncar(act["amenazas"], 85), border=1, new_x="LMARGIN", new_y="NEXT", align="C")
    ...
    pdf.set_text_color(0, 0, 0)
    pdf.set_font("Helvetica", "B", 12)
    pdf.cell(w=0, h=10, text="4. ESTADO GLOBAL DE LA RED", border=0, new_x="LMARGIN", new_y="NEXT", align="L")
