...
Efecto = onto.Efecto
Accion = onto.Accion  # <-- Cambiado de Actividad a Accion
Tarea = onto.Tarea
...
# 6. ACCIONES (Antiguas Actividades)
for act_data in data.get("acciones", []):
    acc = Accion(act_data["id"])
    padre = onto.search_one(iri=f"*{act_data['contribuyeA']}")
    if padre: acc.contribuyeAEfecto.append(padre)
...
for t_data in data.get("tareas", []):
    ...
    if padre: tarea.contribuyeAAccion.append(padre)  # <-- Cambiado a contribuyeAAccion
